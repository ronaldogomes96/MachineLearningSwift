...
//  SimplePerceptron
//
//  Created by Ronaldo Gomes on 12/06/21.
//
...
class Perceptron {
    ...
    func predict(data: [Double]) -> Int {
        ...
        return sigmoid(sum: sumOfWeights)
    }
    ...
    func train(data: [[Double]], targets: [Int]) {
        ...
        for epoch in 0...epochs {
            ...
            for (positionLine, line) in data.enumerated() {
                ...
                let sinal = sigmoid(sum: sumOfWeights)
    ...
    private func sigmoid(sum: Double) -> Int {
        let sigmoid = 1 / (1 + exp(sum))
        return degree(sum: sigmoid)
    }
